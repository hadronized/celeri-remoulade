...
impl Line {
  pub fn render_cmd<'a>(line: &'a Entity<Self>) -> RenderCommand<'a, LinesUniforms> {
    RenderCommand::new(None,
                       true,
                       move |&(_, _, ref inst, ref color, _): &(_, _, UniformUpdate<Transform>, Uniform<[f32; 3]>, Uniform<[f32; 2]>)| {
...
pub fn new_line_entity(line: &Vec<[f32; 3]>, seed: f32) -> Entity<Line> {
  ...
  let color = color_palette([0.5, 0.5, 0.5], [0.5, 0.5, 0.5], [2., 1., 1.], [0.5, 0.2, 0.25], seed*0.005);
  let line = Line {
